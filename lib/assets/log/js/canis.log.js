function CanisLog($canvas, initial) {
	var self = this;
	$canvas.addClass('canis-log');
	this.$canvas = $canvas;
	this.data = initial;
	this.elements = {};
...
CanisLog.prototype.updateData = function() {
	var self = this;
...
CanisLog.prototype.drawInfo = function (info) {
	var self = this;
	var detailConfig = {
		'started': {
			'icon': 'fa fa-play',
...
CanisLog.prototype.drawTasks = function ($wrapperCanvas, tasks, parentTask) {
	var self = this;
...
CanisLog.prototype.updateMessages = function() {
	var self = this;
	if (this.data.messages) {
		...
		jQuery.each(this.data.messages, function(index, message) {
			if (self.renderedMessages[index] === undefined) {
				...
				if (message.data !== null) {
					$("<code />").addClass('expanded-only canis-log-preformatted').html(JSON.stringify(message.data, null, "\t")).appendTo(self.renderedMessages[index]);
				}
...
CanisLog.prototype.renderCanvas = function() {
	...
	this.elements.$left = $("<div />", {'class': 'canis-sidebar'}).appendTo(this.elements.$leftContainer);
	...
	this.elements.$output = $("<div />", {'class': 'panel panel-default canis-log-output'}).hide().appendTo(this.elements.$left);
	...
	this.elements.$output.$body = $("<div />", {'class': 'panel-body canis-log-preformatted'}).appendTo(this.elements.$output);


};

CanisLog.prototype.refresh = function() {
	var self = this;
	var ajax = {};
...
$("[data-log]").each(function() {
	...
	$(this).data('log', new CanisLog($(this), initial));
});