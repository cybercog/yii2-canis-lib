function TealBrowser ($parent, options) {
   ...
   if (this.options.root && !(this.options.root instanceof TealBrowserBundle)) {
      this.options.root = new TealBrowserBundle(this, this.options.root);
   }
   this.$parent = $parent;
   this.bundles = {};
   this.elements = {};
...
TealBrowser.prototype = jQuery.extend(true, {}, TealComponent.prototype);
TealBrowser.prototype.objectClass = 'TealBrowser';

TealBrowser.prototype.select = function(item) {
   this.options.callback(item);
   this.hide();
   this.reset();
};

TealBrowser.prototype.error = function(message) {
   this.reset(false);
   this.elements.$canvas.html('<div class="alert alert-danger">'+ message +'</div>');
};

TealBrowser.prototype.init = function() {
   var self = this;
   this.elements.$container = $("<div />").hide().addClass('teal-browse').appendTo(this.$parent);
   this.elements.$canvas = $("<div />").addClass('teal-browse-canvas').appendTo(this.elements.$container);
...
TealBrowser.prototype.addRequestBundle = function (bundle, callback, request) {
   if (request === undefined) {
      if (!this.request || this.request.executed) {
         this.request = new TealBrowserRequest(this);
      }
...
TealBrowser.prototype.drawBundle = function(bundle) {
   if (bundle instanceof TealBrowserBundle) {
      bundle.draw();
   }
};

TealBrowser.prototype.internalDrawBundle = function(bundle, $element) {
   if (bundle instanceof TealBrowserBundle) {
...
TealBrowser.prototype.draw = function() {
   if (this.options.root) {
      this.drawBundle(this.options.root);
   }
};

TealBrowser.prototype.internalUpdateMarginShift = function(left) {
   this.elements.$canvas.animate({'marginLeft': left}, this.options.section.animationSpeed);
};

TealBrowser.prototype.updateViewport = function() {
...
TealBrowser.prototype.reset = function(draw) {
   if (draw === undefined) {
      draw = true;
   }
   this.stack = [];
...
TealBrowser.prototype.show = function() {
   var self = this;
   this.reset();
...
TealBrowser.prototype.hide = function() {
   var self = this;
   this.elements.$container.slideUp(function() { self.visible = false; self.reset(); });
};

TealBrowser.prototype.appendStackItem = function(bundle, item) {
...
TealBrowser.prototype.handleStack = function(stack, draw) {
   if (stack.length === 0) {
      this.reset();
      return false;
   } else {
      var stackObject = new TealBrowserStack(stack);
...
TealBrowser.prototype.getSectionWidth = function() {
...
TealBrowser.prototype.goToPositionIndex = function(index, shiftViewport) {
...
function TealBrowserBundle (browser, options) {
   var defaultOptions = {
      'id': null,
      'instructions': {},
      'type': 'item',
...
TealBrowserBundle.prototype = jQuery.extend(true, {}, TealComponent.prototype);

TealBrowserBundle.prototype.getInstructions = function() {
...
TealBrowserBundle.prototype.getId = function() {
   return this.options.id;
};
TealBrowserBundle.prototype.undraw = function() {
   this.$element.remove();
   this.$element = null;
   this.listInitialized = false;
   this.rendered = false;
...
TealBrowserBundle.prototype.draw = function() {
   var self = this;
   this.rendered = true;
   this.state = 'list';
   ...
   var $loadElement = this.$loadElement = $("<div />", {'class': 'glyphicon glyphicon-chevron-down teal-browse-load-element'}).hide().appendTo(this.$element);
...
TealBrowserBundle.prototype.drawItems = function(items) {
   var self = this;
   if (items === undefined) {
      items = this.items;
   }
...
TealBrowserBundle.prototype.checkLoader = function() {
   var self = this;
...
TealBrowserBundle.prototype.isLoaded = function() {
...
TealBrowserBundle.prototype.loadBundleResponse = function(bundleResponse, request) {
   var self = this;
   this.fetched = true;
...
TealBrowserBundle.prototype.emptyListNotice = function() {
...
TealBrowserBundle.prototype.updateState = function (state, filterQuery) {
...
TealBrowserBundle.prototype.handleSearch = function() {
   var self = this;
...
TealBrowserBundle.prototype.initializeList = function(search) {
   var self = this;
   ...
   if (self.browser.stack[self.getPosition()-1] !== undefined) {
      ...
      $("<a />", {'href': '#', 'class': 'teal-browse-back list-group-item'}).html('<i class="glyphicon glyphicon-chevron-left pull-left"></i> Back to <em>'+previousStackItem.descriptor+'</em>').appendTo(self.$list).click(function() {
         self.browser.goToPositionIndex(self.getPosition()-1);
         return false;
      });
      if (previousStackItem.isSelectable) {
         $("<a />", {'href': '#', 'class': 'teal-browse-select list-group-item'}).html('<i class="glyphicon glyphicon-check pull-right"></i> Select <em>'+previousStackItem.descriptor+'</em>').appendTo(self.$list).click(function() {
   ...
   if (search) {
      var searchInput = $("<input />", {'type': 'text', 'class': 'teal-browse-filter form-control', 'placeholder': 'Filter...'});
...
TealBrowserBundle.prototype.appendItem = function(item) {
   var self = this;
   this.initializeList();
   if (this.$list === null) { return false; }
...
TealBrowserBundle.prototype.getPosition = function() {
   var self = this;
   var position = false;
   var positionTest = 0;
...
TealBrowserBundle.prototype.fetch = function(callback) {
   if (this.state === 'list' && this.isLoaded()) { return true; }
   this.browser.addRequestBundle(this, callback);
};


TealBrowserBundle.prototype.fetchMore = function() {
   clearTimeout(this.fetchTimer);
   this.fetch(function() {
...
function TealBrowserStack(stack) {
   this.stack = stack;
}

TealBrowserStack.prototype.getStack = function() {
   return this.stack;
};

TealBrowserStack.prototype.getId = function() {
...
TealBrowserStack.prototype.getBundle = function(browser) {
   return new TealBrowserBundle(browser, {
      'id': this.getId(),
      'instructions': this.getInstructions()
   });
};

TealBrowserStack.prototype.getInstructions = function() {
   var instructions = {};
   instructions.id = this.getId();
...
function TealBrowserRequest(browser, options) {
   var defaultOptions = {
      'autoexecute': true,
      'ajax': {
         'dataType': 'json'
...
TealBrowserRequest.prototype.add = function(bundle, callback) {
   if (this.executed) {
      return false;
   }
   var id = bundle.getId();
...
TealBrowserRequest.prototype.execute = function() {
...
TealBrowserRequest.prototype.callback = function(data) {
   var self = this;
...
(function ($) { 
   $.fn.tealBrowser = function (opts) {
   		var $this = this;
      	if ($this.tealBrowserObject === undefined) {
      		$this.tealBrowserObject = new TealBrowser($this, opts);
      	}

         return $this.tealBrowserObject;
   };
}(jQuery));