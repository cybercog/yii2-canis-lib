function TealSearch (parent, options) {
   var self = this;
   this.parent = parent;
   var defaultOptions = {
      'remote': {
      ...
      'templates': {
         'empty': '<div class="teal-search-result teal-search-empty"><h4 class="teal-search-header">No objects matched your query.</h4></div>',
...
TealSearch.prototype.searchItemResult = function(context) {
   var $item = $("<a />", {'class': 'teal-search-result', 'href': '#'}).click(function(e) { e.preventDefault(); });
   if (context.icon !== undefined) {
      var $icon = $("<span />", context.icon).appendTo($item).addClass('teal-search-icon');
   };
   if (context.descriptor !== undefined) {
      var $descriptor = $("<h4 />", {class: 'teal-search-header'}).html(context.descriptor).appendTo($item);
   }
   if (context.subdescriptor !== undefined) {
      ...
      var $subdescriptor = $("<div />", {class: 'teal-search-text'}).html(context.subdescriptor).appendTo($item);
   }
   return $item;
};
...
(function ($) { 
   $.fn.tealSearch = function (opts) {
         var $this = this;
         if ($this.tealSearchObject === undefined) {
            $this.tealSearchObject = new TealSearch($this, opts);
         }

         return $this.tealSearchObject;
   };
}(jQuery));