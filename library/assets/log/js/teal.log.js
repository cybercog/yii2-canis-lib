function TealLog($canvas, initial) {
	var self = this;
	$canvas.addClass('teal-log');
	this.$canvas = $canvas;
	this.data = initial;
	this.elements = {};
...
TealLog.prototype.updateData = function() {
	var self = this;
...
TealLog.prototype.drawInfo = function (info) {
	var self = this;
	var detailConfig = {
		'started': {
			'icon': 'fa fa-play',
...
TealLog.prototype.drawTasks = function ($wrapperCanvas, tasks, parentTask) {
	var self = this;
...
TealLog.prototype.updateMessages = function() {
	var self = this;
	if (this.data.messages) {
		...
		jQuery.each(this.data.messages, function(index, message) {
			if (self.renderedMessages[index] === undefined) {
				...
				if (message.data !== null) {
					$("<code />").addClass('expanded-only teal-log-preformatted').html(JSON.stringify(message.data, null, "\t")).appendTo(self.renderedMessages[index]);
				}
...
TealLog.prototype.renderCanvas = function() {
	...
	this.elements.$left = $("<div />", {'class': 'teal-sidebar'}).appendTo(this.elements.$leftContainer);
	...
	this.elements.$output = $("<div />", {'class': 'panel panel-default teal-log-output'}).hide().appendTo(this.elements.$left);
	...
	this.elements.$output.$body = $("<div />", {'class': 'panel-body teal-log-preformatted'}).appendTo(this.elements.$output);


};

TealLog.prototype.refresh = function() {
	var self = this;
	var ajax = {};
...
$("[data-log]").each(function() {
	...
	$(this).data('log', new TealLog($(this), initial));
});